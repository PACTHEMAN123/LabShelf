...
LabShelf 是一个 **纯数据管理框架**，为 Attention MoE 分离式推理实验提供元数据管理和数据溯源。它不运行实验本身，只负责：
...
```
labshelf.py          单文件 CLI，所有命令入口（无需 pip install）
...
- 添加新数据格式：在 `loaders.py` 的 `_LOADERS` 字典中添加加载函数，在 `labshelf.py` 的 `_detect_format` 中添加扩展名映射，在 `config.yaml` 的 `supported_formats` 中添加格式名。
