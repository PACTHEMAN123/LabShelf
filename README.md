...
LabShelf 不运行实验本身，只负责管理实验元数据、数据文件和处理脚本，维护清晰的 **数据 → 脚本 → 输出** 溯源链。
...
# 2. 添加数据文件（环境/溯源 per-entry 记录，git 信息自动检测）
...
data:                     # 数据注册表（逻辑名 → 文件 + 环境/溯源）
  <logical-name>:
    ...
    environment:          # per-entry 环境信息
      machine: "<machine>"
      gpu: "<gpu>"
    provenance:           # per-entry 代码溯源
      code_branch: "<branch>"
      code_commit: "<commit>"
...
- **实验 = 研究方向**：无日期前缀，每个实验代表一个研究方向
- **环境/溯源 per-entry**：每条数据独立记录实验环境和代码版本
- **脚本与实验解绑**：脚本统一在 `scripts/` 下，可复用于多个实验
- **统一输出**：`output/` 替代 `figures/` + `others/`，不限制输出格式
