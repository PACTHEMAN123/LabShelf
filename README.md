...
AlloScale 不运行实验本身，只负责管理实验元数据、数据文件和可视化脚本，维护清晰的 **数据 → 脚本 → 图表** 溯源链。
...
无需安装 AlloScale 本身，直接运行 `python3 alloscale.py`。
...
# 3. 创建可视化脚本
python3 alloscale.py add-script moe-routing latency_cdf \
  --inputs latency_log \
  --desc "延迟 CDF 分布图"
...
# 5. 运行绘图
python3 alloscale.py plot moe-routing latency_cdf

# 6. 查看实验
python3 alloscale.py list --tag routing
python3 alloscale.py show moe-routing
python3 alloscale.py info moe-routing
```
...
- **单文件 CLI**：无需安装，`python3 alloscale.py` 即用
